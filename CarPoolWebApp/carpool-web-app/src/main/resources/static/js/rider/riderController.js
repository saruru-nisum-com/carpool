/**
 * @Author: Dhiraj singh description: riderController for riderApp. date
 *          created: 11th nov. 20117 date modified: 29th nov. 20117
 */
riderApp.controller(
				'riderController',
				function($scope, $state, localStorageService, riderService,
						$rootScope, $filter) {
							...
							.$on(
									...
									function() {
										if ($scope.riderAutocomplete != undefined) {
											$scope.selectedLocation = $scope.riderAutocomplete
													.getPlace().name;
											var location = $scope.riderAutocomplete
					...
					$scope.getRegisteredRiderData = function() {
						...
						riderService
								.getRegisterRiderData($scope.userId)
								.then(
										function(response) {
					...
					$scope.registerAsRider = function() {
						...
						var mobile = $scope.mobile;// static for now, need to
													// change
						...
						riderService
								.registerAsRider($scope.registerRiderJson)
								.then(
										function(response) {
						...
						riderService
								...
								.then(
										function(successResponse) {
											console
													.log("Rider data updated successfuly"
															+ successResponse);
											$scope.isVisible = true;
										...
										function(errorResponse) {
											console
													.log("Failed to update the rider data."
															+ errorResponse);
										});
